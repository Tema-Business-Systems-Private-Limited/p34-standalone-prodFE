...
export default function PurchaseOrderManager({selectedOrder,setSelectedOrder,isCreate,setCreate,setLoader}) {
  ...
  const [filteredOrders, setFilteredOrders] = useState(vehicleClassList);
  // const [isCreate, setCreate] = useState(false);
  const [error, setError] = useState(selectedOrder?.className.trim()?"This field is mandatory":"");
  const [commonData, setCommonData] = useState([]);


  console.log(vehicleClassList, "this is vehicle class list")
  ...
  const onClickCreateVehicleClass = () => {
