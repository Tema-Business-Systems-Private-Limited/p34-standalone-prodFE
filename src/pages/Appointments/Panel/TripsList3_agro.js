...
class TripsList3 extends React.Component {
 ...
 onGroupValidateNo = () => {
  ...
  onGroupValidateYes = () => {
    ...
    getLockData = (lock, i, opti, itemcode) => {

        if (lock) {

            return (
                <span onClick={() => this.OnLockClicked(i, opti, lock, itemcode)}>
    ...
    getVrDetailsLink(x, i, tmsValidated, itemcode) {
       ...
       // if (x == 1) {
            return (
                <a href="#"
                    onClick={() => this.OnVRDetailBtnClicked(x,i, tmsValidated, itemcode)}><i class="fa fa-info-circle fa-lg" aria-hidden="true"></i>
                </a>
            );
       // }
    }
    ...
    getValidatebtn(valid, lock, index, itemcode) {
        if (!valid && lock) {
            return (
                <a href="#"
                    onClick={() => this.OnValidateClicked(index, itemcode)}><i class="fas fa-check-circle" aria-hidden="true"></i>
                </a>
            );
        }
        else {
    ...
    onConfirmDeleteYes = (index, tripcode) => {
        this.props.onCompleteTripDelete(index, tripcode);
        this.setState({
            addDeleteconfirmShow: false
        })
    }
                  ...
                  <Card className="mb-3">
                                        ...
                                        <Col md="4">
										 ...
										 <div className="custom-control mb-0 mr-4">
                                                                       <Checkbox  color="primary"
                                                                          onChange = {this.daysforTripsCheckbox}
                                                                          checked = {this.props.daysforTripsCheckedIn}/>
                                                                       <Label

                                                                          onClick={() => {
                                                                                                                                              this.setState({
                                                                                                                                                daysforTrips : !this.state.daysforTrips,
                                                                                                                                              });
                                                                                                                                            }}
                                                                                                                                          >
                                                                                                                                            {this.props.t('+/- 30days')}
										  ...
										  <div className="custom-control mb-0 mr-4">
                                                                                                               <Checkbox  color="primary"
                                                                                                                  onChange = {this.alltripsCheckbox}
                                                                                                                  checked = {this.props.alltrips}/>
                                                                                                               <Label
                        ...
                        <div className="reportlist-view tablheight">
                            <table className="table m-0">
                                ...
                                <tbody>
                                    {(filteringTrips && filteringTrips || []).map((trip, i) => (
                                            ...
                                            <td>
                                                {trip.lock ? '' :
                                                    <button className="btn btn-danger btn-sm rounded-0" type="button" data-toggle="tooltip" data-placement="top" title="Delete"
                                                        onClick={() => this.OnDeletebtnClicked(i, trip.itemCode)} disabled={trip.lock}>
                                                        <i class="fa fa-trash"></i>
                                                    </button>}
                                            </td>
                                            <td>
                                                {this.getVrDetailsLink(trip.lock, i, trip.tmsValidated, trip.itemCode)}
                                            </td>
											{this.props.daysforTripsCheckedIn &&
                                        <td>{moment.tz(trip.docdate, '').format('DD-MM-YYYY')}</td>
                                            ...
                                            <td width="4%"><span style={{ fontSize: '12pt'}} >{trip.optistatus ? trip.optistatus == "Optimized" ? this.props.t('Optimized') : nullAndNanChecking(trip.optistatus, 'status') : 'Open'}</span></td>
                                                                                        <td width="3%">
                                                                                            {
                                                                                                this.getLockData(trip.lock, i, trip.optistatus || 'open', trip.itemCode)
                                                                                            }
                                                                                        </td>
                                                                                        <td width="4%">
                                                                                            {this.getValidatebtn(trip.tmsValidated, trip.lock, i, trip.itemCode)}
                                                                                        </td>
