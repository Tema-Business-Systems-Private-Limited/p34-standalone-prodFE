...
const TrailerList = ({ trailerList, handleEditTrailer, handleDeleteClick, loader }) => {
    return (
                ...
                <table className="min-w-full text-left border-collapse">
                    <thead className="bg-gradient-to-r from-gray-100 to-gray-200 text-black uppercase text-sm tracking-wider shadow-sm">
                        <tr className="font-bold text-center">
                            <th className="px-6 py-4 font-semibold">Trailer</th>
                    ...
                    <tbody
                       className="text-gray-800 text-sm divide-y divide-gray-200">
                        ...
                        {
                            loader
                                ? [...Array(trailerList?.length || 7)].map((_, idx) => (
                                ...
                                trailerList.map((trailer, index) => (
                                    <tr key={index} className="hover:bg-blue-50 transition duration-200 text-center odd:bg-gray-50">
                                        <td className="px-6 py-3 whitespace-nowrap">{trailer.trailer}</td>
                                        <td className="px-6 py-3 whitespace-nowrap">{trailer.fcy}</td>
                                        <td className="px-6 py-3 whitespace-nowrap">{trailer.type}</td>
