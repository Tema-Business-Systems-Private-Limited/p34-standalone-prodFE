...
class Dashboard extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      istripCreatedSuccessfully: false,
      AutoGeneratedTripCode: "",
      existingTripCode: [],
      istripOptimisedSuccessfully: false,
      istripLockedSuccessfully: false,
      droppedTrailers: [],
      allowedTrailers: [],
      isDetail: false,
  ...
  colourDivs = (allDrivers, dlist, allTrailers, tlist) => {
    ...
    console.log(
      "T22 after assiging -vehicleDropped",
      this.state.vehicleDropped
    );
  };
  ...
  // auto generate trip using auto popup 
      ...
      for (let i = 0; i < selVeh.length; i++) {
        ...
        if (!sameVehiclesflag && !sflag) {
          ...
          if (stime < etime) {
            ...
            Veh.description = veh.codeyve;
            Veh.max_travel_time = convertHrToSec(veh.maxtotaltrvtime);
            console.log(parseInt(veh.capacities) , parseInt(veh.vol) ,"array values before passing")
            Veh.capacity = [parseInt(veh.capacities) , parseInt(veh.vol)];
      ...
      for (let j = 0; j < SelDocs.length; j++) {
        ...
        if (
          ...
          let wtime =
            ...
            console.log(parseInt(wtime) ,"this is service timeeeeeee check in black optimize button")
...
// console.log(processedData ,"this is processed data from ours side")
      ...
      fetch(uurl, {
        ...
        .then((response) => {
          ...
          if (response.status === 200) {
            ...
            //   console.log("errorMessageerrorMessage", errRes);
            //   const errormessage = errRes.error;
            //   const latLngRegex = /\[(-?\d+\.\d+);(-?\d+\.\d+)\]/;
            //   console.log("hbjdshjuikjhvff", latLngRegex);
            //   const match = errormessage.match(latLngRegex);
            //   console.log("wesdsxj hbjvnfnx", match);
            //   console.log("frbshchb jscdh dc", SelDocs);
            //   if (match !== null) {
            //     // const errorLat = match[1]
            //     // const errorLng = match[2]
            //     const errorLat = parseFloat(match[1]).toFixed(5);
            //     const errorLng = parseFloat(match[2]).toFixed(5);
            //     console.log("rfhbdcsdgbchjbhdh", errorLat, errorLng);

            //     console.log("frbshchb jscdh dc", SelDocs);
            //     const tolerance = 0.00001; // Define your tolerance level

            //     const matchingDocNums = SelDocs.filter((item) => {
            //       const roundedLat = parseFloat(item.lat.toFixed(5));
            //       const roundedLng = parseFloat(item.lng.toFixed(5));
            //       console.log("sendmethatcode", roundedLat, roundedLng);
            //       return (
            //         Math.abs(roundedLat - errorLat) < tolerance &&
            //         Math.abs(roundedLng - errorLng) < tolerance
            //       );
            //     })
            //       // .map(item => item.docnum)
            //       .map(
            //         (item) => `${item.bpcode} ${item.bpname} - ${item.docnum}`
            //       )
            //       .join("\n");

            //     console.log("Matching docnums:", matchingDocNums);
            //     this.setState({
            //       optimizeFailedDocuments: matchingDocNums,
            //       // errorMessage: this.state.optimizeFailedDocuments,
            //       // loader: false,
            //     });
            //   } else if (match === null) {
            //     let bpcode = SelDocs[0].bpcode;
            //     let bpname = SelDocs[0].bpname;
            //     let docnum = SelDocs[0].docnum;
            //     this.setState({
            //       optimizeFailedDocuments: `${bpcode} ${bpname} - ${docnum}`,
            //     });
            //   }
            //   console.log(
            //     "optimizeFailedDocuments",
            //     this.state.optimizeFailedDocuments
            //   );
            //   console.error("API error response:", errormessage); // Log the error response
       
            // });

            this.setState({
              errorMessage: "Selected documents' coordinates are either out of range or incorrect.",
              addAlertShow: true,
              loader: false,
            });
          }
  ...
  // setting state for auto popup open and close
  openPopupAuto = (val) => {
    this.setState({
      openAutoPopup: val,
    });
  ...
  submitTrips = async (trips, type, callsType) => {
    console.log("Tzzz inside submit trips", callsType);
    // this.setState({ loader: true });
    await fetch(`${apiUrl}/api/v1/transport/trips`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(trips),
    })
      .then(async (response) => {
        console.log("Tzzz inside after trip - response", response);
        this.handleErrors(response);
      })
      .then(async () => {
        console.log("Tzzz inside submit Trips", this.state.date);
        await this.handleDateChange(this.state.date);
      })
      .then(async () => {
        this.setState({ checkedTrip: false, isDetail: false });
        if (type === "Open") {
          console.log("Tzzz inside submit trips open if", type);
          this.setState({ istripCreatedSuccessfully: true });
        } else if (type === "Optimized") {
          console.log("Tzzz inside submit trips optimize if", type);
          this.setState({ istripOptimisedSuccessfully: true });
        }
        this.notifySucess("Trip Added/Updated Sucessfully");
        console.log("Tzzz inside after success");
        if (callsType === "MultipleCalls") {
          this.ConfirmToproceedforOptimisation();
        }
        if (callsType === "Locking") {
          this.ConfirmToproceedforLocking(trips);
        }
      })
      .catch((error) => {
  ...
  ConfirmToproceedforLocking = async (trips) => {
    console.log("Tzzz inside locking trips are ", trips);
    var TripIndex;
    var tripsPan = this.state.tripsPanel;
    for (var i = 0; i < tripsPan.length; i++) {
      if (trips[0].itemCode == tripsPan[i].itemCode) {
        TripIndex = i;
      }
    }
    await this.onLockRecord(TripIndex);
  };

  ConfirmToproceedforOptimisation = async () => {
    var LatestIndex, LatestTrip, LatestTripCode;
    console.log("TZZZ tripcodes  prev exist", this.state.existingTripCode);
    var tempTripList2 = this.state.tripsPanel;
    var tempTripsCode = this.state.existingTripCode;
    for (let j = 0; j < tempTripList2.length; j++) {
      let ttrip2 = tempTripList2[j];
      if (tempTripsCode.includes(ttrip2.itemCode)) {
        console.log("TZZZ tripcodes  true exist", ttrip2.itemCode);
      } else {
        LatestIndex = j;
        LatestTrip = ttrip2;
        LatestTripCode = ttrip2.itemCode;
      }
    }
    console.log("TZZZ inside before optimisation", LatestIndex);
    console.log("TZZZ inside before optimisation", LatestTripCode);
    await this.tempOptimizeRoute(LatestTrip, LatestIndex, "Locking"); // Step 2
  };

  confirmCOLTrip = async (trip, from) => {
    this.setState({ loader: true });
    this.setState({
      istripOptimisedSuccessfully: false,
      istripCreatedSuccessfully: false,
    ...
    var tempTripList = this.state.tripsPanel;
    var tripCodes = [];
    var LatestIndex, LatestTrip, LatestTripCode;
    for (var i = 0; i < tempTripList.length; i++) {
      let ttrip = tempTripList[i];
      console.log("TZZZ trip is", ttrip);
      tripCodes.push(ttrip.itemCode);
    }
    this.setState({ existingTripCode: tripCodes });

    await this.confirmTrip(trip, from, "", "", "MultipleCalls"); // Step 1
  };

  // 3 exec
  /*
   confirmCOLTrip2 = async (trip, from) =>  {
       console.log("TZZZ inside confirmCOLTrip");
        this.setState({ istripOptimisedSuccessfully: false , istripCreatedSuccessfully : false, istripLockedSuccessfully : false });
           var tempTripList = this.state.tripsPanel;
           var tripCodes = [];
           var LatestIndex, LatestTrip, LatestTripCode;
           for (var i = 0; i < tempTripList.length; i++) {
         ...
         console.log("TZZZ inside before confirmCOLTrip");


    await  this.confirmTrip(trip, from);                     // Step 1
        console.log("TZZZ inside after confirmCOLTrip");
           console.log("TZZZ inside after confirmCOLTrip - state",this.state.istripCreatedSuccessfully);

       if(this.state.istripCreatedSuccessfully)
       {
          var tempTripList2 = this.state.tripsPanel;
           console.log("TZZZ latest TripList is",tempTripList2);
            console.log("TZZZ tripcodes is",tripCodes);
           for (let j = 0; j < tempTripList2.length; j++) {
               let ttrip2 = tempTripList2[j];
               if(tripCodes.includes(ttrip2.itemCode)) {
                   console.log("TZZZ tripcodes  true exist",ttrip2.itemCode);
               }
               else {
                  LatestIndex = j;
              ...
              console.log("TZZZ inside before optimisation");
          //   this.tempOptimizeRoute(LatestTrip, LatestIndex , CallType);  // Step 2
       }
       if(this.state.istripOptimisedSuccessfully)
       {
       console.log("TZZZ inside before Locking");
           this.onLockRecord(LatestIndex);
       }
   }
   */
  ...
  tempOptimizeRoute = (data, selectedI, CallType) => {
    let passedData = data;
    this.setState({ loader: true });
    ...
    let c, d, m, v;
    let date = new Date();
    console.log("T223 inside loading - vehiclepanel", this.state.vehiclePanel);
    this.state.vehiclePanel &&
      this.state.vehiclePanel.vehicles.length > 0 &&
          ...
          if (results) {
            ...
            if (data && legs && data.stops < results[0].legs.length) {
              ...
              if (
                ...
                //   this.getValues(routesSchedule, optiindex, false , data, SeletedI);
  ...
  confirmTrip = async (trip, route, routesSchedule, newGeoData, callsType) => {
    // this.setState({ loader: true });
    console.log("T555 inside confirm trip =", routesSchedule);
    console.log("T555 inside routeSchedule  =", route);
    console.log("T555 inside newGeoData  =", newGeoData);
    var processType = "";
    if (
      ...
      await this.submitTrips(itemTrips, processType, callsType);
      var currDropsPanel = this.state.dropsPanel;
  ...
  TriplistforAllCheckedin = (allflag) => {
    ...
    let filteringTrips;
    if (!allflag) {
      filteringTrips = currTripsPanelList.filter((trip) => {
  ...
  TriplistforCheckedin = (dayflag) => {
    ...
    var newEndDate = moment(edate).format("YYYY-MM-DD");
    console.log(
      "T222 inside changeDate - 30daysflag",
      this.state.checked30daystrips
    );
    if (dayflag) {
      console.log("T222 inside checked and true");
      fetchTripsPanelwithRange(
        this.state.selectedMultipleSites,
        newStartDate,
        newEndDate
      )
  ...
  OptimisePreparation = (data) => {
    ...
    let jsonUrl = `/json?computeBestOrder=true&routeRepresentation=summaryOnly&computeTravelTimeFor=all&routeType=shortest&avoid=unpavedRoads&travelMode=truck&vehicleMaxSpeed=${
      data.vehicleObject.maxspeed
    }&vehicleWeight=${data.vehicleObject.capacities}&vehicleLength=${
      data.vehicleObject.length / 100
    }&vehicleWidth=${data.vehicleObject.width / 100}&vehicleHeight=${
      data.vehicleObject.heigth / 100
    }&vehicleCommercial=true&vehicleLoadType=otherHazmatGeneral&vehicleEngineType=combustion`;
    let prevTripsDist = 0;
    let prevTripsTime = 0;
    var summaryData = [];
    var optiindex = [];
...
*/
  ...
  componentDidMount() {
    console.log(
      "DAsh index-  this.props.location.state.date",
  ...
  handleSiteChange = (selectedOption) => {
    console.log("TMMM site change", selectedOption);
    ...
    fetchAPI(selectedOption, currDate)
      .then(([res1, res2, res3,res4]) => {
        ...
        console.log(res4 ,"check route codes")
      })
      .then(() => {
        this.updateTopBar();
        this.refreshSite();
...
*/
  ...
  updateTrip = (trip) => {
    var SelectedDocs = [];
    if (trip && trip.length > 0 && trip[0].dropObject) {
    ...
    if (trip && trip.length > 0 && trip[0].pickupObject) {
  ...
  onTripDelete = (index, docnum, vtype, vcode) => {
    ...
    console.log(
      "T222 inside app after delete button clicked- currentGeoData",
      currentGeoData
    );
    console.log(
      "T222 inside app after delete button clicked- currentMarkers",
      currentMarkers
    );
    console.log(
      "T222 inside app after delete button clicked- currDropsPanel",
      currDropsPanel
    );
    ...
    if (currentGeoData[index].panelType == "pickup") {
      ...
      for (var i = 0; i < pickUps.length; i++) {
        if (pickUps[i].docnum == docnum) {
          ...
          trip.doc_capacity = trip.doc_capacity - pickUps[i].netweight;
          trip.doc_volume = trip.doc_volume - pickUps[i].volume;
        }
      }
    ...
    trip.per_capacity = ((trip.doc_capacity / trip.tot_capacity) * 100).toFixed(
      2
    );
  ...
  onCompleteTripDelete = (index, tripcode) => {
    console.log("PPPP on trip delete parameters", index, tripcode)
    var tripsPanel = this.state.tripsPanel;
    var trips = [];
    var trip = tripsPanel[index];
    trips.push(trip);
  ...
  deleteTrip = (trips, index) => {
    console.log("deleteTripdeleteTrip", trips, index)
    var tripsPanel = this.state.tripsPanel;
    tripsPanel[index].lock = true;
    const currDate = moment(this.state.date).format("YYYY-MM-DD");
    let value = this.state.selectedMultipleSites;
  ...
  getRouteSchedulerApp = (
    ...
    console.log(
      "T5151 inside GetRouteScheduler - routesSchedule",
      routesSchedule
    );
    var data = [];
    var newGeoData = [];
    if (auto) {
      ...
      if (from === "Trips") {
        ...
        console.log("T5151 Tripsdata else else");
        console.log(
          "T5151 inside GetRouteScheduler - geoData",
          this.state.geoData
        );
    ...
    console.log("T5151 Tripsdata else end");
    this.setState({ geoData: newGeoData });
    this.setState({ routeSchedulerTime: routesSchedule });
    console.log(
      "T5151 inside GetRouteScheduler - final routesSchedule",
      routesSchedule
    );
    this.confirmTrip(
      routesSchedule.trips,
      "route",
      routesSchedule,
      newGeoData,
      callType
  ...
  updateonlyTripsPanel = (i) => {
    ...
    tripsPanels[i].timelineInterval = slectTrip.timelineInterval;
    console.log(
      "inside updateTripsPanel - totalobject- timeline =",
      slectTrip.timelineInterval
    );
    selectedTrips = slectTrip.selectedTripData;
    console.log(
      "inside updateTripsPanel - totalobject- selectedtrip =",
      selectedTrips
    );
    trailers = slectTrip.trailers;
    equipments = slectTrip.equipments;
  ...
  getRouteSchedulerAppbackup = (routesSchedule, optimisedindex, auto) => {
    console.log("inside getRouteshcler App - opti index", optimisedindex);
    console.log(
      "T555 inside get RouteScheduelr - routeSchduler",
      routesSchedule
    );
    var data = [];
    var newGeoData = [];
    if (auto) {
      ...
      var data = [];
      var newGeoData = [];
      console.log("T555 inside get RouteScheduelr", this.state.geoData);
      this.state.geoData.map((geo, geoIndex) => {
        routesSchedule.routesData.map((route, routeIndex) => {
          if (geoIndex === routeIndex) {
            data = { ...geo, ...route };
    ...
    if (DocCount > 0) {
      ...
      for (let j = 0; j < docsPanel.length; j++) {
        ...
        if (
          ...
          var FromArr = doc.fromTime.split(" ");
          var ToArr = doc.toTime.split(" ");
...
*/
  ...
  //       console.log(
  //         "T1212 same pricess used Drivers =",
  //         sameProcessUsedDriversList
  //       );
  ...
  //                     activeDrivers[dl].driverid
  //                   )
  //                 ) {
  //                   console.log(
  //                     "T1212 active randon unused driver assigned =",
  //                     defaultDriver
  //                   );
  ...
  //               console.log(
  //                 "T1212 active randon driver assigned =",
  //                 defaultDriver
  //               );
  //             }
  //           } else {
  ...
  //                 console.log(
  //                   "T1212 active randon first driver assigned =",
  //                   defaultDriver
  //                 );
  //                 defaultDriver = activeDrivers[dl].driverid;
  //                 defaultDrivername = activeDrivers[dl].driver;
  ...
  submitRoutesforTripsCreation = (routes, site, docsPanel, selDrivers) => {
    var RouteprocessedData = [];
    var sameProcessUsedDriversList = [];
    var TripsfromRoutes = [];
    console.log("OSRM Auto Routes data are", routes);
          ...
          for (let d = 0; d < docsPanel.length; d++) {
            ...
            if (currDoc.docnum === docno) {
              ...
              if (currDoc.doctype === "PRECEIPT") {
                ...
                let tempqty = 0, tempuom = '';
                currDoc.products && currDoc.products.length > 0 &&
...
console.log("T1212 active drivers =", VehicleObject);
      if (VehicleObject.driverid === " " || VehicleObject.driverid === "" || VehicleObject.driverid === null ) {
        //assign some random driver from the active drivers
        var activeDrivers = this.state.vehiclePanel.drivers;
        var tempTripPanel = this.state.tripsPanel;
        var sflag = false;
        ...
        console.log("T1212 active drivers =", activeDrivers);
        if(selDrivers?.length > 0) {
             console.log("T1212 active drivers if=");
      ...
      totalWeight = (totalWeight % 1 === 0)
        ? totalWeight.toString() : totalWeight.toFixed(2)
      var today = new Date();
      var execdate = today.getDate();
  ...
  render() {
   ...
   button = <LoginButton onClick={handleClick} />;
    ...
    // console.log(this.state.sites,"from TMSNEW dash index")
    ...
    return (
      <React.Fragment>
              ...
              <div>
                <FormControlLabel
                  ...
                  control={
                    <Switch
                      color="primary"
                      checked={this.state.autoOptimisationflag}
                ...
                <Row className="mt-3">
                  ...
                  <Col md="12">
                    <AddUpdateTrip1
                      ...
                      confirmCOLTrip={this.confirmCOLTrip}
        ...
        ></Alert>
        {this.state.setTripClosedError && (
          <DisplayInformationIconDetails
            show={this.state.setTripClosedError}
            onInfoIconHide={this.warningWindowClose}
            data={this.state.setTripsClosedErrorMessage}
            warning={true}
          ></DisplayInformationIconDetails>
        )}
        {this.state.setDistError && (
          <DisplayInformationIconDetails
            show={this.state.setDistError}
            onInfoIconHide={this.warningWindowClose}
            data={this.state.setDistErrorMessage}
            warning={true}
          ></DisplayInformationIconDetails>
        )}
        {this.state.setTimeError && (
          <DisplayInformationIconDetails
            show={this.state.setTimeError}
            onInfoIconHide={this.warningWindowClose}
            data={this.state.setTimeErrorMessage}
            warning={true}
          ></DisplayInformationIconDetails>
        )}
        {this.state.setOptiStatusError && (
          <DisplayInformationIconDetails
            show={this.state.setOptiStatusError}
            onInfoIconHide={this.warningWindowClose}
            data={this.state.setOptimizationMessage}
            warning={true}
          ></DisplayInformationIconDetails>
        )}
        {this.state.setModelTime && (
          <DisplayInformationIconDetails
            show={this.state.setModelTime}
            onInfoIconHide={this.warningWindowClose}
            data={this.state.setModalTimeMessage}
            warning={true}
          ></DisplayInformationIconDetails>
        )}
        {this.state.setOptimizationFailedStatus && (
          <DisplayInformationIconDetails
