...
class TripsList3 extends React.Component {
  ...
  OnDisputeValidateTrip = (index) => {
    this.setState({
      confirmMessage: "Are you Sure you want to dispute the Validated Trip",
  ...
  getValidatebtn(valid, lock, index, validatedflg) {
    if (!valid && lock && !validatedflg) {
      return (
        <a href="#" onClick={() => this.CheckValiationStatus(index)}>
          <i class="fas fa-check-circle" aria-hidden="true"></i>
        </a>
      );
    } else if (valid && lock && !validatedflg) {
      return (
        <a href="#" onClick={() => this.OnDisputeValidateTrip(index)}>
          <i class="fas fa-check-circle" aria-hidden="true"></i>
        </a>
      );
    } else {
  ...
  setStartTime(trip) {
    if (trip.optistatus === "Optimized" || trip.optistatus === "optimized") {
      return tConvert(splitTime(trip.startTime));
    } else {
      let sameTrips = [];
      let loadHrs;
      if (this.props.tripsList && this.props.tripsList.length > 0) {
        ...
        if (sameTrips.length > 0) {
          ...
          return tConvert(formatTime(time));
        } else {
          return tConvert(splitTime(trip.vehicleObject.starttime));
        }
      } else {
        return tConvert(trip.vehicleObject.starttime);
      }
    }
  }
  /*
              ...
              <table className="table m-0">
                <thead>
                  <tr className="">
                    ...
                    {this.props.daysforTripsCheckedIn && <th> Route Date </th>}
                      ...
                      >
                        ...
                        <td className="pl-2">
                          <input
                            ...
                            name="tripsCheckBox"
                            onClick={() =>
                              this.OnCheckBoxClicked(i, trip.itemCode)
                            }
                          />
                        </td>
                        <td>
