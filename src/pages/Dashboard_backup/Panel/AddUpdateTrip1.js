...
import ConfirmCOL from './ConfirmCOL';
import ConfirmPreparationList from './ConfirmPreparationList';
import Alert from './Alert';
import PreloadConfirm from './PreloadConfirm';
...
class AddUpdateTrip1 extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            addConfirmCOLShow: false,
            addConfirmPreListShow: false,
            addPreloadConfirm: false,
            droppedData: {},
            preTrip: {},
            addAlertShow: false,
            errorMessage: '',
    ...
    drop(event, eventType) {
        ...
        console.log("preload before 1", eventType)
        /*
        ...
        if (eventType === 'createVehicle') {
            ...
            if (!(this.props.trips && this.props.trips[0] && (this.props.trips[0].lock || this.props.trips[0].tmsValidated))) {
                ...
                else if (eventType === 'preloadTrailer') {
                    console.log("preload before 2")
                    type = 'trailer';
                    this.setState({
                        addPreloadConfirm: false
                    })
                ...
                if (eventType === 'createVehicle') {
                    ...
                    type = "trailer";
                    data = this.state.selectedTrailer;
                    console.log("preload before 3")
                }
                else if (eventType === 'updateVehicle') {
                ...
                if (type === 'vehicle') {
                    ...
                    if (this.props.checkedTrip && !(eventType === 'updateVehicle')) {
                        ...
                        if (VehicleCompatability) {
                            ...
                            trip.tot_capacity = data.capacities;
                            trip.tot_volume = data.vol;
                    ...
                    if (data.miscpickflg == 2) {
                        ...
                        let result = this.addDrop(currentTrip, data, trip, 2, 0);
                            ...
                            if (data.pairedDoc != undefined && data.pairedDoc != '') {
                                for (var i = 0; i < this.props.currDropsPanel.pickUps.length; i++) {
                                    if (data.pairedDoc === this.props.currDropsPanel.pickUps[i].docnum) {
                                        ...
                                        this.addPickup(currentTrip, this.props.currDropsPanel.pickUps[i], trip, 2, 1);
                        ...
                        this.addDrop(currentTrip, data, trip, 1, 0);
                        ...
                        if (data.pairedDoc != undefined && data.pairedDoc != '') {
                            for (var i = 0; i < this.props.currDropsPanel.pickUps.length; i++) {
                                if (data.pairedDoc === this.props.currDropsPanel.pickUps[i].docnum) {
                                    currentTrip = this.props.trips;
                                    this.addPickup(currentTrip, this.props.currDropsPanel.pickUps[i], trip, 1, 1);
                                    this.props.disableDivs(i, "pickup", this.props.currDropsPanel.drops[i].docnum);
                                }
                            }
                        }
                    ...
                    if (trip.code === "") {
                        ...
                        this.setState({
                            errorMessage: '{this.props.t("Vehicle_first0")}',
                            addAlertShow: true,
                            error: true,
                        })
                    ...
                    if (trip.code === "") {
                        ...
                        if (!(eventType === 'preloadTrailer')) {

                            docsCompatability = false;
                            this.setState({
                                addPreloadConfirm: true,
                                confirmMessage: "Are you sure , you want to use Pre-Loaded Trailer",
                                currentTrip: trip,
                                preloadedTrailerIndex: event.dataTransfer.getData("index"),
                            ...
                            if (trip.drops > 0) {
                                ...
                                for (var i = 0; i < trip.dropObject.length; i++) {
                                    if (data.prod && data.prod.includes(trip.dropObject[i].prodCode)) {
                                        docsCompatability = true;
                                    }
                                    else {
                                        console.log("Prod is not compatabilit with Trailer")
                                        docsCompatability = false;
                                        this.setState({
                                            errorMessage: "Trip documents - products are not compatible with the Preloaded trailer",
                                            addAlertShow: true,
                                            error: true,
                    ...
                    console.log("preload 1", docsCompatability)
                    if (docsCompatability === true) {
                        if (trip.trailerLink && trip.trailerLink != 'Yes') {
                            status = false;
                            this.setState({
                                errorMessage: '{this.props.t("Veh_single_trail_comp")}',
                                addAlertShow: true,
                                error: true
                            });
                        } else if ((!this.props.checkedTrip && this.props.trailers.length >= 1) ||
                            (this.props.checkedTrip && this.props.trips && this.props.trips[0] && this.props.trips[0].trialerObject
                                && this.props.trips[0].trialerObject.length == 1)) {
                            status = false;
                            this.setState({
                                errorMessage: '{this.props.t("cant_one_trailer")}',
                                addAlertShow: true,
                                error: true
                            });
                            ...
                            this.setState({
                                errorMessage: '{this.props.t("trailtype_vehclass_ass")}',
                                addAlertShow: true,
                                error: true
                            });
                        }
                            ...
                            console.log("preload inside last else trailer");
                            this.props.colourDocDivs(data.trailer);
                ...
                if (status) {
                    if (!(eventType === 'updateVehicle')) {
                        ...
                        if (eventType === 'preloadTrailer') {
                            this.props.disableDivs(this.state.preloadedTrailerIndex, type, data.docnum);
                        }
                        else {
    ...
    addDrop = (currentTrip, data, trip, validationCheck, from) => {
        ...
        console.log("TKKK current trip ", trip);
        console.log("TKKK current data ", data);
        /*
   
            if(trip.drops > 0 && from === 1) {
              console.log("TKKK current trip if c",trip.drops);
              dropCompatability = false;
                ...
                console.log("TKKK current trip else c ",trip.drops);
            }
       */
        console.log("Tkkk trip trailer count =", trip.trailers)
        if ((trip.trailerLink && trip.trailerLink === 'Yes') && trip.trailers <= 0) {
            dropCompatability = false;

            error = "TrailerFirstMulti";
            this.setState({
                errorType: 'TrailerFirstMulti'
            });

        }

        // Customer Vehicl class compatability with Vehile Class from  Vehicle
        if (dropCompatability) {

            console.log("TKKK current data - Veh class6", data.vehClassList)
            if (data.vehClassList.length > 0) {
                var tempVehClass = data.vehClassList.split(" ");
                console.log("TKKK current data - Veh class6 temp", tempVehClass);
                var veh_vehclass_cus_vehClass_flg = tempVehClass.includes(trip.vehicleObject.catego)
        ...
        if (dropCompatability) {
            ...
            if (trip.tot_capacity < trip.doc_capacity + data.netweight) {
                dropCompatability = false;
        ...
        if (dropCompatability) {
            if (trip.tot_volume < trip.doc_volume + data.volume) {
                dropCompatability = false;
                error = "VolumeFull";
                this.setState({
        ...
        if (dropCompatability == true) {
            ...
            trip.doc_capacity = trip.doc_capacity + data.netweight;
            trip.doc_volume = trip.doc_volume + data.volume;

            if (trip.doc_capacity > 0) {
                trip.per_capacity = ((trip.doc_capacity / trip.tot_capacity) * 100).toFixed(2);
            }
            if (trip.doc_volume > 0) {
                trip.per_volume = ((trip.doc_volume / trip.tot_volume) * 100).toFixed(2);
            }
            ...
            console.log("TKKK error", error);
            if (error === 'moreDrops') {
                this.setState({
                    errorMessage: 'We cant add morethan one Delivery to Trip',
                    addAlertShow: true,
                    error: true
                });
            }
                ...
                this.setState({
                    errorMessage: 'Vehicle Capacity is Full , cant add New Documents',
                    addAlertShow: true,
                    error: true
                });
            }
            else if (error === 'TrailerFirstMulti') {

                this.setState({
                    errorMessage: 'Please add Trailer First ,as it contains a Multi unit Vehicle',
                    addAlertShow: true,
                    error: true
                });
            }
                ...
                this.setState({
                    errorMessage: 'Vehicle Volume is Full , cant add New Documents',
                    addAlertShow: true,
                    error: true
                });
            }
                ...
                this.setState({
                    errorMessage: 'Vehicle CLASS in Vehicle and VehicleCLASS at Customer doesnt Match',
                    addAlertShow: true,
                    error: true
                });
            }
                ...
                this.setState({
                    errorMessage: 'We cant add morethan one Delivery to Trip - else',
                    addAlertShow: true,
                    error: true
                });
            }
    ...
    addPickup = (currentTrip, data, trip, misctype, from) => {
        ...
        if (misctype == 1) {
            ...
            console.log("TKKK pickup", trip)

            /*
                   if(trip.pickups > 0 && from === 1) {
                      console.log("TKKK pickup if",trip.pickups)
           
                                          pickupError = "morepickups";
                                          pickupCompatability = false;
        ...
        if (pickupCompatability) {
            ...
            else if (pickupError === 'morepickups') {
                this.setState({
                    errorMessage: 'we cant add morethan one Receipt to Trip',
                    addAlertShow: true,
                    error: true
                });
            }
            ...
            else if (pickupError === 'CapacityFull') {
                this.setState({
                    errorMessage: 'Vehicle Capacity is Full , cant add New Documents',
                    addAlertShow: true,
                    error: true
            ...
            else if (pickupError === 'TrailerFirstMulti') {
            ...
            else if (pickupError === 'VolumeFull') {
                this.setState({
                    errorMessage: 'Vehicle Volume is Full , cant add New Documents',
                    addAlertShow: true,
                    error: true
    ...
    addTrailer = (trip, data, code, from) => {
        ...
        trip.tot_capacity = trip.tot_capacity + data.maxloams;
        trip.tot_volume = trip.tot_volume + data.maxlovol;
    ...
    deleteTrailer = (i) => {
        ...
        /* updating the capacity and volumes
     trip.tot_capacity = trip.tot_capacity - removedTrailer.maxloams;
     trip.tot_volume = trip.tot_volume - removedTrailer.maxlovol;
     trip.per_capacity = ((trip.doc_capacity / trip.tot_capacity) * 100).toFixed(2);
     trip.per_capacity = ((trip.doc_volume / trip.tot_volume) * 100).toFixed(2);
     */
        ...
        this.calculate_percentage(removedTrailer);
    }

    calculate_percentage = (removedTrailer) => {
    ...
    onConfirmCOLClick = (trip) => {
        ...
        console.log("T333 inside confirm Trip - drop Volume is", dropVolume);
        console.log("T333 inside confirm Trip - pickup Volume is", pickupVolume);
        ...
        if (trip.driverId !== "") {
            console.log("T333 inside confirm trip 1 ", trip);

            if (trip.vehicleObject.vol < (dropVolume + pickupVolume) ||
                ...
                /*
                console.log("T333 inside confirm volume 2 ",dropVolume + pickupVolume);
                console.log("T333 inside confirm volume 3 ",trip.vol);
              if (trip.vehicleObject.vol <= (dropVolume + pickupVolume)) {
                  message = `Trip Volume (${dropVolume + pickupVolume}) is greater-than Max Vehicle Volume(${trip.vehicleObject.vol}) \n`;
              }
              ...
              */
                message = 'Are you sure you want to confirm , Optimise , Lock  the trip?';
                let warn = false;
    ...
    onConfirmCOLYes = (trip) => {
        console.log("inside confirm YEs");
        this.props.confirmCOLTrip(this.state.currentTrip, 'Open');
        this.setState({
            addConfirmCOLShow: false,
            quantities: 0,
            equipments: []
    ...
    onConfirmPreparationListNo = () => {
        ...
        this.addDrop(this.state.PreCurrentTrip, this.state.droppedData, this.state.preTrip, 1, 0);
        this.setState({
            addConfirmPreListShow: false,
            droppedData: {},
            preTrip: {},
    ...
    confirmPreload = (data, trip) => {

        let PreloaddocsCompatability = true;
        ...
        /*
          ...
          console.log("T111 inside trailer- trip- stops > 0",trip);
            // trailer and document relation
                                                   for( var i=0;i<trip.dropObject.length;i++){
                                                       if (data.prod && data.prod.includes(trip.dropObject[i].prodCode)) {
                                                                         PreloaddocsCompatability = true;
                                                                                                  }
                                                                                                  else {
                                                                                                    console.log("Prod is not compatabilit with Trailer")
                                                                                                      PreloaddocsCompatability = false;
                                                                                                      errorStatus = true;
                                                                                                      this.setState({
                                                                                                                             errorMessage: "Trip documents - products are not compatible with the Preloaded trailer",
                                                                                                                             addAlertShow: true,
        ...
        if (!errorStatus) {
            var currentTrip = this.props.trips;
            console.log("T111 inside error status failed else trailer");
            this.props.colourDocDivs(data.trailer);
            let code = trip && trip.code
            this.addTrailer(trip, data, code, 'Trailer');
        ...
        console.log("inside confirm Preload YEs trailer", data);
        console.log("inside confirm Preload YEs - trip", trip);
        //    this.props.confirmCOLTrip(this.state.currentTrip, 'Open');
        this.setState({
            addPreloadConfirm: false,
        })
    ...
    getTotalData = (selectedTrips, type) => {
        ...
        if (type === 'w')
            return parseInt(tw) + ' ' + wu
        else if (type === 'v')
            return parseInt(tv) + ' ' + vu
        else if (type === 'q')
            return tq + ' ' + 'UN'
        else
            return '0'
    ...
    render() {
        ...
        console.log("inside render currentTrip", this.state.currentTrip);
            ...
            <Card class="col-md-12 pt-2 pb-0 pr-1 pl-1"
                ...
                <CardBody>
                    <div class="middlesection">
                                        ...
                                        {(currTrips || []).map((trip, i) => {
                                            ...
                                            return (
                                                <tr key={i}>
                                                    ...
                                                    <td width="10%">
                                                        <ProgressBar3
                                                            tot={trip.tot_capacity}
                                                            doc={trip.doc_capacity}
                                                            per={trip.per_capacity}
                                                            from='Capacity'
                                                    ...
                                                    <td width="5%">{trip.totalObject && trip.totalObject.selectedTripData.length > 0 ? this.getTotalData(trip.totalObject.selectedTripData, 'w') : 0}</td>
                                        ...
                                        if (currTrips.length > 0) {
                                            ...
                                            >Reset</button> */}
                                                    ...
                                                    <div style={{ paddingLeft: "20px" }}>
                                                        <button type="button" class="btn btn-primary btn-sm mt-2"
                                                            onClick={() => this.onConfirmCOLClick(actualTrip)}
                                                            disabled={actualTrip.lock}
                                                        >
                                                            <i class="fa fa-pen"></i>{this.props.t('C->O->L')}
                                                        </button>
                        ...
                        ></Confirm>
                        <ConfirmCOL
                            show={this.state.addConfirmCOLShow}
                            onHide={this.onConfirmCOLNo}
                            confirmCOLTrip={this.onConfirmCOLYes}
                            trip={this.state.currentTrip}
                            confirmMessage={this.state.confirmMessage}
                            warning={this.state.warning}
                            warningAlert={this.state.warningAlert}
                        ...
                        ></DisplayNotes>
                        <PreloadConfirm
                            show={this.state.addPreloadConfirm}
                            onHidePreloader={this.onPreloadConfirmNo}
                            confirmPreload={this.drop}
                            trip={this.state.currentTrip}
