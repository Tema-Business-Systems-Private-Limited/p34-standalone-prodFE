...
class AlertSummary extends React.Component {
  ...
  render() {
    ...
    const errorNotesArray = this.props.errorNotesArray;
    return (
      <Modal
        {...this.props}
        size="lg"
      ...
      >
        <Modal.Header className='modal-header-bg'>
          <Modal.Title id="contained-modal-title-vcenter">
            <span className='text-white'>Summary , Transaction ID :   {this.props.processID}</span>
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
      ...
      {errorNotesArray?.length > 0 && (
        <>
          ...
          <hr />
          {errorNotesArray.map((notemsg, index) => (
            <div
              key={index}
              style={{
                color: "black",
              ...
              dangerouslySetInnerHTML={{ __html: notemsg }}
            />
          ))}
        </>
      )}
          ...
          {errorMessagesArray.length > 0 ? (
            errorMessagesArray.map((msg, index) => (
              // <div key={index} style={{ color: "black", fontSize: "16px", marginBottom: '10px', whiteSpace: 'pre-line' }}>{msg}</div>
                  <div
      key={index}
      style={{
        color: "black",
