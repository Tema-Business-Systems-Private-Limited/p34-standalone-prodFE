...
function Timeline(props) {
    ...
    const [optimizeFailedDocuments, setOptimizeFailedDocuments] = React.useState('')
    ...
    const getEndDate = () => {
        if (!(props.data.optistatus === "Open") && props.data.endDate) {
            return moment.tz(props.data.endDate, '').format('MM-DD-YYYY');
        }
                ...
                .then(function (response) {
                    ...
                    if (response.status === 200) {
                        ...
                        response.json().then(errRes => {
                            const errorMessage = errRes.detailedError.message;

                            const latLngRegex = /Waypoint \d+ \((-?\d+\.\d+), (-?\d+\.\d+)\)/;
                            ...
                            if (match !== null) {
                                ...
                                let docnum = data.totalObject.selectedTripData[0].docnum;
                                setOptimizeFailedDocuments(`${bpcode} ${bpname} - ${docnum}`);
                            }
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{
                                        ...
                                        moment(props.data.docdate).format("MM-DD-YYYY")}</td>
                                </tr>
                                <tr>
