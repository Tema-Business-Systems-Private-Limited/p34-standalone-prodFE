...
class AddUpdateTrip1 extends React.Component {
  ...
  drop(event, eventType) {
    ...
    /*
                ...
                */
    // console.log("T111 inside drag eventtype =", event, eventType);
    if (eventType === "createVehicle") {
      ...
      if (
        ...
        if (type === "vehicle") {
          ...
          // console.log("Trip details are at drops", trip);
          if (trip.code === "") {
            // console.log("Trip details are at drops if", trip);
            status = false;
            this.setState({
              errorMessage:
                "The vehicle is missing, please add vehicle before proceeding",
              addAlertShow: true,
              error: true,
            });
          } else {
            if (
              ...
              if (data.prelistCode !== " ") {
                // console.log("T000 if");
                // console.log("T000 Prelistcode =", data.prelistCode);
                const message =
                  "Would you like to select all Pick ticket documents of this Preparation list?";
                const warn = false;
                ...
                // console.log("T000 after setState");
              } else {
          ...
          if (trip.code === "") {
            ...
            this.setState({
              errorMessage:
                "The vehicle is missing, please drag and drop it first.",
              addAlertShow: true,
              error: true,
            });
          } else {
            if (trip.alldrivers === 2) {
              ...
              if (
                ...
                this.setState({
                  errorMessage:
                    "The selected driver is not associated with the trip vehicle",
                  addAlertShow: true,
                  error: true,
                });
              } else {
  ...
  addDrop = (currentTrip, data, trip) => {
    console.log("DAta insdie drop 1", currentTrip);
    console.log("DAta insdie drop 2", data);
    console.log("DAta insdie drop 3", trip);

    // console.log(this.state.PreCurrentTrip ,"checking precurrenttrip")
    let dropCompatability = true;
    let excludeValidations = false;
    let error = "";

    // console.log(trip ,"this is current trip")
    if (currentTrip.length > 0) {
      trip = currentTrip[0];
      currentTrip = [];
    }
    ...
    // console.log(trip.vehicleObject ,"vehicle obj check")
    let currDocQty = 0;
    data.products.map((prod) => {
      // console.log("Data insdie drop prod", prod);
      currDocQty = currDocQty + parseInt(prod.quantity);
    });

    // console.log("Data insdie drop - trip", trip);
    // console.log("Data insdie drop - vehicle", currentVehicle);
    // console.log("Data insdie drop - docqty", currDocQty);
    // // console.log("Data insdie drop - max order cnt", currentVehicle.maxordercnt);
    // console.log("Data insdie drop - trip count", trip.stops);
    // to check max order count compatability

    // if (currentVehicle?.maxordercnt < trip?.stops + 1) {  // commented on 280125
    //   // console.log("Data insdie drop - if", currentVehicle.maxordercnt);
    //   dropCompatability = false;
    //   excludeValidations = true;
    //   //  error = "Trip Order Count is exceeded with Vehicle Maximum Order Count";
    //  // error = "OrderCount";
    //   this.setState({
    //     confirmMessage:
    //       "Trip Order Count is exceeded with Vehicle Maximum Order Count. Please confirm to proceed to add document to the trip exclude all the validations",
    ...
    //   });
    // } else {
    //   // console.log("Data insdie drop - else");
    // }


    // commented on 230125
    // if (dropCompatability && !excludeValidations) {
    //  // // console.log("Data insdie drop - trip qty", currentVehicle.maxqty);
    // //  // console.log("Data insdie drop - drop qty", trip.doc_qty);
    // //  // console.log("Data insdie drop - drop qty", currDocQty);
    // //  // console.log("Data insdie drop - sum qty", trip.doc_qty + currDocQty);
    //   if (
    //     currentVehicle.maxqty <
    //     parseInt(trip.doc_qty) + parseInt(currDocQty)
    //   ) {
    //     dropCompatability = false;
    //     excludeValidations = true;
    //     //  error = "Trip quantity Count is exceeded with Vehicle Maximum quantity Count";
    //     error = "QunatityCount";
    //     this.setState({
    //       errorType: "QunatityCount",
    //       confirmMessage:
    //         "Trip quantity Count is exceeded with Vehicle Maximum quantity Count. Please confirm to remove all the validations and add to the trip",
    //       addConfirmToProceedShow : true,
    //       currentTrip : trip,
    //        droppedData : data,
    //      // error: true,
        ...
        if (
            ...
            if (
              ...
              this.setState({
                errorType: "Driver",
                 ...
                 addConfirmToProceedShow : true,
                 currentTrip : trip,
                 confirmMessage : "Driver is not compatability to the customer. Please confirm to exclude all the validations to add document to the trip"
              });
            }
          } else {
            dropCompatability = true;
          ...
          this.setState({
            ...
            droppedData : data,
                  addConfirmToProceedShow : true,
                             currentTrip : trip,
                             confirmMessage : "Trailer is not compatability to the customer. Please confirm to exclude all the validations to add document to the trip"
    ...
    //                               addConfirmToProceedShow : true,
    //                                          currentTrip : trip,
    //                                          confirmMessage : "Product is not compatability to the customer. Please confirm to exclude all the validations to add document to the trip"
    //         });
    //         break;
    //       }
    //     }
          ...
          this.setState({
            errorType: "Veh_VehClass_Cus_VehClass",
                          droppedData : data,
                                addConfirmToProceedShow : true,
                                           currentTrip : trip,
                                           confirmMessage : "Vehicle Class is not compatability to the customer assigned vehicle class. Please confirm to exclude all the validations to add document to the trip"
          });
        }
      }
    }
                  ...
                  this.setState({
                    errorType: "productTrailer",
                                  droppedData : data,
                                        addConfirmToProceedShow : true,
                                                   currentTrip : trip,
                                                   confirmMessage : "Product  is not compatability to the Trailer assigned to it. Please confirm to exclude all the validations to add document to the trip"
                  });
                  break;
                }
              }
...
excludeValidations = true;
          ...
          this.setState({
            ...
            error: true,
                          droppedData : data,
                                addConfirmToProceedShow : true,
                                           currentTrip : trip,
                                           confirmMessage : "Vehicle is not compatability to the customer. Please confirm to exclude all the validations to add document to the trip"
          });
        } else {
          dropCompatability = true;
        }
    ...
    if (dropCompatability == true && !excludeValidations) {
      // console.log("TYYY insde drop data", data);
      // console.log("TYYY insde drop trip", trip);
      data.panelType = "drop";
      ...
      if (error === "product") {
        ...
        this.setState({
          errorMessage:
            "Trip Order Count is exceeded with Vehicle Maximum Order Count",
          addAlertShow: true,
          error: true,
        });
        ...
        this.setState({
          errorMessage:
            "Trip quantity count is exceeded with Vehicle Maximum Quantity Count",
          addAlertShow: true,
          error: true,
        });
      } else {
      ...
      )
      {
        if (trip.vol <= dropVolume + pickupVolume) {
          message =
            ...
            }  M3) exceeds the maximum allowed vehicle volume capacity(${
              trip.vol
            } M3))`;
        }
        if (
          ...
          message =
            message +
            `The Trip has more orders(${
              trip.pickupObject.length + trip.dropObject.length
            }) than the vehicles maximum order count (${
              trip.vehicleObject.maxordercnt
        ...
        if (
          ...
          message =
            ...
            `Trip Weight (${
              pickupCapacities + dropCapacities
            } KG) is greater than the maximum allowed vehicle weight capacity(${
              trip.vehicleObject.capacities
            } KG))`;
        }
        ...
        if (trip.pickupObject.length <= 0 && trip.dropObject.length <= 0) {
          message =
            "Pick-up or drops not assigned to the trip. Are you sure you want to delete the trip ?";
          warn = false;
        }
  ...
  OnProceedToExcludeValidations = () => {
      ...
      // console.log("On proceed current Trip", trip);
      // console.log("On proceed droppedData", data);
      ...
      return (
        <table>
          ...
          <tr>
            ...
            <td>{this.props.selectedTripData.poscode}</td>
            <td>
            ...
            <td>
              {this.getTotalQty(this.props.selectedTripData.products, "d")}{" "}
            </td>
          </tr>
        </table>
      );
  ...
  getTotalQty = (prodList, from) => {
    ...
    prods &&
      prods.length > 0 &&
      prods.map((prod) => {
        // console.log("T555 insdie getTotlQty convQty", prod.convQty);
    ...
    let Qtywithuom = parseFloat(TotQty).toFixed(2) + " " + uom;
  ...
  unlockTrip = (tmsValidated) => {
    if (tmsValidated) {
      this.setState({
        errorMessage:
          "The trip is already validated, you cannot unlock the trip.",
        addAlertShow: true,
      });
    } else {
  ...
  handleSiteChange(value, trip, type) {
    ...
    if (type === "end") {
      this.setState({
        confirmMessage: "Are you sure, want to change the site ?",
                    ...
                    {(currTrips || []).map((trip, i) => {
                      ...
                      selectedTrip?.forEach( trip => {
                        trip?.products?.forEach( product => {
                          totalProductsQuantity += parseInt(product.quantity, 10);
                        });
                      });
                      ...
                      return (
                        <tr key={i}>
                          ...
                          <td width="5%">{trip.travelTime} Hours</td>
                          <td width="5%">
                           {!trip.totalDistance || isNaN(trip.totalDistance) ? 0 : Math.round(trip.totalDistance)} KM
                          </td>
                          <td width="5%">
