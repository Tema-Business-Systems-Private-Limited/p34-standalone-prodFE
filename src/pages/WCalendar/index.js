...
class WCalendar extends React.Component {
  ...
  componentDidMount() {
    ...
    Promise.all([
      fetch(
        "http://solutions.tema-systems.com:8062/api/v1/report/usrsites?user=" +
          user.username
      ),
      fetch(
        "http://solutions.tema-systems.com:8062/api/v1/report/tripslist?date=" +
          date
      ),
    ])
      .then(([sites, mapTrips]) => {
  ...
  onMapDateAndSiteChange = (seldate, sites) => {
    Promise.all([
      fetch(
        `http://solutions.tema-systems.com:8062/api/v1/report/tripslistbyDateandSite?date=${seldate}&site=${sites}`
      ),
    ])
      .then(([trips]) => {
  ...
  onMapDateChange = (seldate, camefrom) => {
    ...
    if (this.state.selectedSitesArr.length > 0 && camefrom !== "uncheck") {
      ...
      Promise.all([
        fetch(
          `http://solutions.tema-systems.com:8062/api/v1/report/tripslist?date=${seldate}`
        ),
      ])
        .then(([trips]) => {
          return Promise.all([trips.json()]);
  ...
  OnSiteSelection = (selSite, from) => {
    ...
    Promise.all([
      fetch(
        `http://solutions.tema-systems.com:8062/api/v1/report/vehicle?site=${selSite}`
      ),
      fetch(
        `http://solutions.tema-systems.com:8062/api/v1/report/route?site=${selSite}&start=${startDate}&end=${endDate}`
      ),
    ])
      .then(([vehicles, data]) => {

        console.log("vehicles and data in w-calandar", vehicles,data)
      ...
      .then(([vehicles, data]) => {
        ...
        console.log("tripDetails inside wcalendar fetch", data)
      });
  };
  ...
  RefreshDataforWeek = () => {
    ...
    let startDate = moment(new Date(this.state.weekStartDate)).add(1, 'days').format("YYYY-MM-DD");
    ...
    if (this.state.selectedSite) {
      Promise.all([
        fetch(
          `http://solutions.tema-systems.com:8062/api/v1/report/route?site=${this.state.selectedSitesArr}&start=${startDate}&end=${endDate}`
        ),
      ])
        .then(([data]) => {
