...
isTimeRangeWithinAnotherRange = (outerStartTime, outerEndTime, innerStartTime, innerEndTime) => {
  ...
  return innerStartDate >= outerStartDate && innerEndDate <= outerEndDate;
    ...
    BPartnerBadgeLink = (clientcode, dtype) => {
                                          ...
                                          if (docmvt == 'DLV') {
                                               url = "http://x3tms.tema-systems.com/syracuse-main/html/main.html?url=/trans/x3/erp/TMSNEW/$sessions?f=GESBPC/2//M/"+clientcode;
                                                                   // content = "<div id = 'bodyContent'><a href=" + url + " target='_blank'>" +docno + "</a>"</div></div>";
                                                               return (
                                                                        <a href={url} target='_blank' >{clientcode}  </a>
                                               );
                                          }
                                          if (docmvt == 'PICK') {
                                          ...
                                          if (docmvt == 'PRECEIPT') {
                                               url = "http://x3tms.tema-systems.com/syracuse-main/html/main.html?url=/trans/x3/erp/TMSNEW/$sessions?f=GESBPS/2/M//"+clientcode;
                                                                   // content = "<div id = 'bodyContent'><a href=" + url + " target='_blank'>" +docno + "</a>"</div></div>";
                                                               return (
                                                                        <a href={url} target='_blank' >{clientcode} </a>
                                                               );
 ...
 GetTiminingWithinRange1 = (acceptedProps) => {
   ...
   console.log("TTTT props are tooltip time", acceptedProps);
    let arrtime1 = acceptedProps.arvtime , deptime1 =  acceptedProps.deptime, Tfromtime1, Ttotime1;
     let resultMatch1 = false, timeExist1 = true;
...
for (var ti1 = 0; ti1 < Tfromtime1.length; ti1++) {
  //console.log(strArray[i]);
   resultMatch1 =  this.isTimeRangeWithinAnotherRange(Tfromtime1[ti1],Ttotime1[ti1],arrtime1,deptime1);
  if(resultMatch1 === true)
     break;
}
     }
...
console.log("TTTT props are tooltip time time exist", timeExist1);
 ...
 GetTiminingWithinRange = (acceptedProps) => {
   ...
   console.log("TTTT props are", acceptedProps);
    let arrtime = acceptedProps.arvtime , deptime =  acceptedProps.deptime, Tfromtime, Ttotime;
...
for (var ti = 0; ti < Tfromtime.length; ti++) {
    ...
    render() {
       console.log("inside RouteMap-  geodata",this.props.geoData);
        ...
        return (
                       ...
                       <div class="routeMapOuter" >
                <div className="mapouter topsection">
                    <div class="reportlist-view" style={{ display: this.props.showList ? "block" : "none" }}>
                            ...
                            <thead style={{ textAlign: 'left' }}>
                                <tr>
                                    ...
                                    <th width="6%">{this.props.t('ServiceTime')}</th>
                                     <th >Deliverable</th>                                    <th width="6%">{this.props.t('WaitingTime')}</th>
                                    ...
                                    return (
                                        ...
                                        >
                                                       ...
                                                       {(popupState) => (
                                                         <React.Fragment>
                                                           ...
                                                           anchorOrigin={{ vertical: 'bottom', horizontal: 'right'  }}
                                                            ...
                                                            >
                                                             <MenuItem onClick={() => this.displayDocumentMessage(data.docnum, data.ptheader)}>{this.props.t('Document Instructions')}</MenuItem>
                                                            {data.doctype && data.doctype === 'DLV' ?
                                                            <MenuItem onClick={() => this.displayCarrierMessage(data.docnum, data.CarrierMessage , 'carrier')}>{this.props.t('Carrier Instructions')}</MenuItem>
                                                             :'' }
                                                             {data.doctype && data.doctype === 'DLV' || data.doctype === 'PICK' ?
                                                               <MenuItem onClick={() => this.displayCarrierMessage(data.docnum, data.loaderMessage , 'loader')}>{this.props.t('Loader Instructions')}</MenuItem>
                                                             :'' }
                                                           </Menu>
                                                         </React.Fragment>
                                                       )}
                                             ...
                                             <td width="3%">
                                                                                            {data.lock || (this.props.trips && this.props.trips[0] && this.props.trips[0].lock) ? '' :
                                                                                                <button class="btn btn-danger btn-sm rounded-0" type="button" data-toggle="tooltip" data-placement="top" title="Delete"
                                                                                                    onClick={() => this.onConfirmClick(i, data.docnum, data.vehicleCode, data)} disabled={data.lock}>
                                            ...
                                            <td width="6%" name="itemCode">{data.vehicleCode}</td>
                                             <td>{
                                                                                this.GetTiminingWithinRange(data)
                                                                                }</td>
                                            ...
                                            <td width="6%"><a href="#"
                                                                                                                                                    onClick={() => this.onInfoClick(logisticDetails, data.docnum, data.doctype)}>{data.bpcode}
                                                                                                                                                    </a></td>
                                            <td width="6%">
                                              <a href="#"
                                                                                         onClick={() => this.onInfoClick(logisticDetails, data.docnum, data.doctype)}>{data.bpname}
                                                                                         </a>
                                            </td>
 ...
 <td>{this.displayDeliverableStatus(data.docnum)}</td>
                    ...
                    <div class="gmap_canvas" style={{ display: this.props.showList ?  "none" : "block"  }}>
                        <div
                            id="google-map"
