...
class AddUpdateTrip1 extends React.Component {
  ...
  componentDidMount() {
    console.log("CDM CompDM trips ");
    console.log("CDM CompDM trips", this.props.trips);
  }

  componentDidUpdate(prevProps) {
    if (this.props.id !== prevProps.id) {
      //API_Call(this.props.id)
      console.log("CDM updte CompDM trips if");
    }
    console.log("CDM updte CompDM trips");
    console.log("CDM update CompDM trips", this.props.trips);
  }
  ...
  handleDateChange = (date) => {
    /*
       // console.log("T223 inside Timeine- date ",date);
        ...
        if (this.props.trips && this.props.trips > 1) {
            let endHr;
            let endMin;
            this.props.tripsPanel.map((tripPanel) => {
                if (tripPanel.code === this.props.trips.code) {
                    if (tripPanel.trips === this.props.trips - 1) {
  ...
  drop(event, eventType, vehicle) {
    console.log("T211 inside drop event, add updateTrip", event);

    console.log("T211 inside drop event, add updateTrip  type", eventType);
    /*
    ...
    if (eventType === "createVehicle") {
      ...
      if (
        ...
        console.log("T211 inside current trip", currentTrip);
        ...
        if (!(eventType === "updateVehicle")) {
          ...
          console.log("checking data in the wwetufgyeshifu if", data)
        } else {
          console.log("T211 updateVehicleData", this.state.updateVehicleData);
          type = "vehicle";
          data = this.state.updateVehicleData;
          console.log("checking data in the wwetufgyeshifu else", data)
        ...
        if (type === "vehicle") {
          ...
          console.log("T211 Inside App- vehicle", data);
          if (this.props.checkedTrip && !(eventType === "updateVehicle")) {
            console.log("checking data in the wwetufgyeshifu last if", data)
            this.setState({
              checkedTripShow: true,
              vehicleMessage: "Do you want to replace the existing vehicle with this new vehicle for the trip?",
              enableOk: false,
              updateVehicleData: data ? data : vehicle,
            ...
            if (tempCurrentData.length > 0) {
              console.log("T211 inside tempdata", tempCurrentData);
            ...
            if (VehicleCompatability) {
              ...
              console.log("T211 data before push", trip);
...
//Vehicle level validation
            ...
            if (trip.alldrivers === 2) {
              ...
              if (
                ...
                this.setState({
                  errorMessage:
                    "Selected Driver is not associated with current Vehicle",
                  addAlertShow: true,
                  error: true,
                });
              } else {
  ...
  addDrop = (currentTrip, data, trip) => {
    ...
    if (dropCompatability) {
      if (
        trip.maxpalletcnt <
        parseInt(trip.noofpackgs, 10) + parseInt(data.noofpackgs, 10)
      ) {
        if (trip.trialerObject.length > 0) {
          dropCompatability = false;

          error = "MaxPalletCount_exceed";
          this.setState({
            errorType: "MaxPalletCount_exceed",
          });
        } else {
          dropCompatability = false;

          error = "TrialerEmpty_MultiUnitVeh_Pallet";
          this.setState({
            errorType: "TrialerEmpty_MultiUnitVeh_Pallet",
          });
        }
      }
    }
    ...
    if (dropCompatability) {
      console.log("weight =", trip.doc_capacity);
      if (
        trip.tot_capacity <
    ...
    if (dropCompatability == true) {
      ...
      trip.noofpackgs = trip.noofpackgs + data.noofpackgs;
      currentTrip.push(trip);
      ...
      } else if (error === "MaxPalletCount_exceed") {
        this.setState({
          errorMessage:
            "Pallet Count is exceeded Max Pallet Count of Trailer & Vehicle",
          addAlertShow: true,
          error: true,
        });
      } else if (error === "TrialerEmpty_MultiUnitVeh_Pallet") {
        this.setState({
          errorMessage:
            "Pallet Count is exceeded and also Trailer is empty, Please add Trailer for this Multi Unit Vehicle",
          addAlertShow: true,
          error: true,
        });
  ...
  addTrailer = (trip, data, code, from) => {
    ...
    trip.tot_capacity = Number(trip.tot_capacity) + Number(data.maxloams);
    trip.tot_volume = Number(trip.tot_volume) + Number(data.maxlovol);
    trip.maxpalletcnt = Number(trip.maxpalletcnt) + Number(data.maxpalletcnt);

    if (trip.doc_capacity > 0) {
      trip.per_capacity = (
        (trip.doc_capacity / trip.tot_capacity) *
        100
      ).toFixed(2);
    }
  ...
  onConfirmYes = (trip) => {
    ...
    this.props.confirmTrip(trip, "Open");
    this.setState({
      addConfirmShow: false,
      quantities: 0,
      equipments: [],
  ...
  getTotalData = (selectedTrips, type, trip) => {
    let tw = 0,
      tv = 0,
      tq = 0,
      wu = "",
    ...
    if (selectedTrips?.length) {
      selectedTrips
        .filter(doc => doc.doctype !== 'BREAK')   // ⬅️ exclude the “BREAK” rows
        .forEach(doc => {
          tw += Number(doc.netweight);
          tv += Number(doc.volume);
          tp += parseInt(doc.noofpackgs, 10);
          // wu  = doc.weightunit;    
          // vu  = doc.volume_unit;
          wu = trip?.uom_capacity || doc.weightunit;
          vu = trip?.uom_volume || doc.volume_unit;
          tq += Number(this.getTotalQty(doc.products));
        });
    }

    if (type === "w") return tw.toFixed(2) + " " + wu;
    else if (type === "v") return tv.toFixed(2) + " " + vu;
    else if (type === "q") return tq + " UN";
    else if (type === "p") return tp;
    else return "0";
  };

  render() {
    console.log("inside add trips", this.props.trips);
    ...
    console.log("actualTrip inside addUpdateTrip1", actualTrip);
    return (
      <Card
        ...
        <CardBody>
          ...
          <div class="middlesection">
                ...
                <table class="table">
                  <thead>
                    <tr>
                      ...
                      <th width="3%">{this.props.t("Departure")} </th>
                      <th width="3%">{this.props.t("Arrival")}</th>
                      <th width="3%">{this.props.t("Seq")} #</th>
                      <th width="5%">{this.props.t("TravelTime")}</th>
                      <th width="5%">{this.props.t("Distance")}</th>

                      <th wisdth="5%">{this.props.t("Total Weight")}</th>
                  ...
                  <tbody>
                    {((currTrips && currTrips) || []).map((trip, i) => {
                            ...
                            <a
                              ...
                              onClick={() =>
                                this.onEquipmentClick(
                                  actualTrip.equipmentObject
                                )
                              }
                            >
                              <u>{this.equipmentData(trip)}</u>
                            </a>
                          </td>
                          <td>
                            {trip.lock ? (
                              ...
                              >
                                ...
                                <div
                                  style={{ paddingTop: "10%", color: "green" }}
                                >
                                  {moment
                                    .tz(this.props.data.docdate, "")
                                    .format("YYYY-MM-DD")}
                                </div>
                                <div
                                  style={{ paddingTop: "10%", color: "blue" }}
                                >
                                  {trip.startTime}
                                </div>
                              </div>
              ...
              <div>
                {(() => {
                  if (currTrips.length > 0 && !currTrips[0].lock) {
                    console.log("hmmm current trip is", currTrips);
                    return (
                      <>
                        <Timeline
                          ...
                          codeExecution={this.props.codeExecution}
                          autofromselection_nextBilloins={this.props.autofromselection_nextBilloins}
            ...
            <PreloadConfirm
              ...
              confirmPreload={this.confirmPreload}
              trip={this.state.currentTrip}
