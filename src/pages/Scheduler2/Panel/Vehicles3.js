      ...
      <TabPane tabId="Vehicles">
        ...
        <div class="reportlist-view tableCustomFixHead1">
            ...
            <tbody>
              {(this.props.curVehicleList || []).map((vehicle, i) => (
                <tr
                  ...
                  key={"vehicle" + i}
                  style={{ backgroundColor: this.getColor(vehicle.color), cursor: (this.props.trips && this.props.trips[0]?.lock) === false ? 'pointer' : 'default'  }}
                  onDoubleClick={() => this.props.handleVehicleDoubleClick(vehicle)}
                >
                  <td>{vehicle.codeyve}</td>
                  <td>{vehicle.name}</td>
                  <td>{vehicle.startdepotn}</td>
                  <td>{vehicle.enddepotname}</td>
                  <td>{vehicle.maxpalletcnt}</td>
                  ...
                  <td>
                    {vehicle.capacities} {vehicle.xweu}
                  </td>
                  <td>
                    {vehicle.vol} {vehicle.xvol}
                  </td>
                  <td>{vehicle.maxordercnt}</td>
                  <td>
                    {vehicle.maxtotaldist} {distunts}
                  </td>
                  <td>
                    {formatTime(convertHrToSec(vehicle.maxtotaltime))} Hrs
                  </td>
                  <td>
                    {formatTime(convertHrToSec(vehicle.maxtotaltrvtime))} Hrs
                  </td>
          ...
          <DisplayInformationIconDetails
            ...
            dataName="vinfo"
            dataType="object"
            vehicle={this.state.selectedVeh}
          ></DisplayInformationIconDetails>
        </div>
      </TabPane>
    );
