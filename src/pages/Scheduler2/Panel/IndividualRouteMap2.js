...
class IndividualRouteMap2 extends React.Component {
  ...
  render() {
    ...
    if (
      ...
      this.props.vrdata &&
      this.props.triplock == true
    ) {
      console.log("inside IndividualMap , lock");
      this.props.tripsList.map((tripList) => {
        if (
          tripList.code === this.props.vrdata.codeyve &&
          this.props.vrdata.xnumpc === tripList.itemCode
        ) {
          // points.push({})
      ...
      this.props.bl_tripsList &&
      this.props.bl_tripsList.lock == false
    ) {
    ...
    let selectedData;
    console.log("TMap tripslist", this.props.tripsList);
    console.log("TMap vrdata", this.props.vrdata);
    console.log("TMap triplock", this.props.triplock);
    if (
      this.props.tripsList &&
      this.props.vrdata &&
      this.props.triplock == true
      ...
      this.props.tripsList.map((tripList) => {
        if (
          ...
          if (tripList.totalObject && tripList.totalObject.selectedTripData) {
            tripList.totalObject.selectedTripData.map(
              (selectedTrips, index) => {
                selectedData = tripList.totalObject.selectedTripData;
                points.push({
                  id: index,
                  title: selectedTrips.bpname,
                  lat: selectedTrips.lat,
                  lng: selectedTrips.lng,
                  type: selectedTrips.doctype,
      ...
      if (
        this.props.bl_tripsList.totalObject &&
        this.props.bl_tripsList.totalObject.selectedTripData
      ) {
        this.props.bl_tripsList.totalObject.selectedTripData.map(
          (selectedTrips, index) => {
    ...
    if (Object.keys(arrDetails.length > 0) && selectedData) {
      ...
      if (
        !(
          depDetails.lat === arrDetails.lat && depDetails.lng === arrDetails.lng
        )
      ) {
    ...
    return (
      <TabPane tabId="VRMaps">
        <Card>
          <CardTitle
            className="float-left h4 mb-0 pt-2"
            style={{ color: "#5664d2", paddingLeft: "20px" }}
          >
            <span>Maps</span>
          </CardTitle>
