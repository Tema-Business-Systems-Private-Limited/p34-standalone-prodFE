...
class Drivers3 extends React.Component {
    ...
    handleDoubleClick = (driver) => {
        if (this.props.trips && this.props.trips[0] && this.props.trips[0].lock === false) {
            // Only update if lock is false
            const updatedTrip = {
                ...this.props.trips[0],  // Spread the existing trip object
                driverId: driver.driverid,  // Update driverId
                driverName: driver.driver,  // Update driverName
            };

            // Create a new array with the updated object (keeping the original object at index 0)
            const updatedTripArray = [updatedTrip];

            // Assuming `updateTrip` is a function passed down as a prop to update the trip in the parent component.
            this.props.updateTripWithDriver(updatedTripArray);
        }
    }
                        ...
                        <tbody>
                            {(this.props.curDrivers || []).map((driver, i) => (
                                <tr id={'driver' + i}
                                    ...
                                    style={{ backgroundColor: this.props.allAllowedDrivers == '2' ? '#feff99' : this.getBgcolor(driver.driverid, driver.color), cursor: (this.props.trips && this.props.trips[0]?.lock) === false ? 'pointer' : 'default' }}
                                    key={'driver' + i}
                                    onDoubleClick={() => this.handleDoubleClick(driver)}
                                >
