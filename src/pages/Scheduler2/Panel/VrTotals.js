...
*/
    ...
    if (trip.lock) {
      ...
      if (
        ...
        selectedDocs = trip.totalObject.selectedTripData.filter(doc => doc.doctype !== "BREAK");
      ...
      for (i = 0; i < selectedDocs.length; i++) {
        ...
        if (selectedDocs[i].movtype == "PICK") {
          pickupwei = parseFloat(selectedDocs[i].netweight !== null ? selectedDocs[i].netweight : 0);
          pickupvol = parseFloat(selectedDocs[i].volume !== null ? selectedDocs[i].volume : 0);
        } else {
          dropwei = parseFloat(selectedDocs[i].netweight !== null ? selectedDocs[i].netweight : 0);
          dropvol = parseFloat(selectedDocs[i].volume !== null ? selectedDocs[i].volume : 0);
        }
        tdropwei = tdropwei + dropwei;
        tpickupwei = tpickupwei + pickupwei;
        tdropvol = tdropvol + dropvol;
    ...
    let veicleVolume = trip.lock ? VehicleVolume : parseFloat(trip?.vehicleObject?.vol).toFixed(2);

    let loadingMass =nullAndNanChecking(weight_per, "vrStops");

    let pickupWeights = nullAndNanChecking(tpickupwei, "vrStops").toFixed(2);

    let dropsVolume=   nullAndNanChecking(parseFloat(tdropvol).toFixed(2), "vrStops");

    let loadingVolume = nullAndNanChecking(volume_per, "vrStops")

    let pickupVolume = nullAndNanChecking(tpickupvol, "vrStops").toFixed(2);
