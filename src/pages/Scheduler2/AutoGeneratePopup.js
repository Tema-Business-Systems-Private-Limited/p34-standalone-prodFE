 ...
 props.autofromselection_nextBilloins(
  ...
  useEffect(() => {
    ...
    let dropsList = [],
      pickUpsList = [];
    if (
      props.dropsPanel && props.dropsPanel.length > 0
      // (props.dropsPanel.drops?.length > 0 || props.dropsPanel.pickUps?.length > 0)
    ) {
      for (let jj = 0; jj < props.dropsPanel.length; jj++) {
        let doc = props.dropsPanel[jj];

        if ((doc.dlvystatus == "8" || doc.dlvystatus == "0")  && doc.movtype === "DROP") {
          docsPanel.push(doc);
          dropsList.push(doc);
        }
      }

      for (let jk = 0; jk < props.dropsPanel.length; jk++) {
        let doc = props.dropsPanel[jk];

        if ((doc.dlvystatus == "8" || doc.dlvystatus == "0") && doc.movtype === "PICK") {
          docsPanel.push(doc);
      ...
      <Card >
        <CardHeader style={{ backgroundColor: "#044C84", color: "white" }} >
          <CardTitle style={{ fontSize: "25px" }}>
            Please select vehicles, drivers, and documents for the Auto-Generation of Trips.
          </CardTitle>
        </CardHeader>
        <CardBody>
          <>
                  ...
                  >
                    ...
                    <Row className="flex">
                      <Col>
                        ...
                        >
                          {activeTab === "Vehicles" ? "Vehicles" : "Drivers / Sales Reps"}
                        </h5>
                      </Col>
