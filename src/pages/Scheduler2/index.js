  ...
  autofromselection_nextBilloins = async (SelDocs, selVeh, selDrivers) => {
    ...
    if (selVeh.length > 0 && SelDocs.length > 0) {
      ...
      for (let i = 0; i < selVeh.length; i++) {
        ...
        // Veh.breaks = breakArray;
        let continousdrivingBreak = {
          max_continuous_driving: 3600, // 2hrs of driving continously
          layover_duration: 900, // 15 min of break
          include_service_time: false // 12hrs of break
        }
        Veh.drive_time_layover_config = continousdrivingBreak // continous break based on driving
        // Veh.profile = "minivan";
        VehList.push(Veh)
      ...
      SelDocs.forEach((item) => {
        if (item.doctype === 'PRECEIPT' && item.docnum) {
          ...
          var fromflag = false
          var toflag = false
          if (item.fromTime && item.fromTime.length > 0) {
            FromArr = item.fromTime.split(' ')
            fromflag = true
          }
          var ToArr
          if (item.toTime && item.toTime.length > 0) {
          ...
          if (item.pairedDoc && item.pairedDoc.trim() !== '') {
            ...
            let tempserTTime = parseInt(convertHrToSec(item.serviceTime)) + parseInt(convertHrToSec(item.waitingTime))
            receiptsObj[item.docnum] = {
              id: id,
              location_index: loc_index, // Replace with real coordinates
              // service: convertHrToSec(item.serviceTime),
              service: tempserTTime,
      ...
      SelDocs.forEach((item) => {
        if (item.doctype !== 'PRECEIPT' && item.docnum) {
          ...
          var fromflag = false
          var toflag = false
          if (item.fromTime && item.fromTime != " " && item.fromTime.length > 0) {
            FromArr = item.fromTime.split(' ')
            fromflag = true
          }
          var ToArr
          if (item.toTime && item.toTime != " " && item.toTime.length > 0) {
            ToArr = item.toTime.split(' ')
            toflag = true
          }
        ...
        for (var j = 0; j < trips[i].dropObject.length; j++) {
          for (var k = 0; k < trips[i].dropObject[j].products.length; k++) {
            Drop_prodCount += parseInt(
              trips[i].dropObject[j].products[k].quantity === null ? 0 : trips[i].dropObject[j].products[k].quantity
            )
          }
        }
      }
...
*/
  ...
  submitRoutesforTripsCreationOSRMManually = async (
    ...
    for (let k = 0; k < routes.length; k++) {
      ...
      console.log("logging all costs 1087", ftotalCost,ffixedcost,fovertimecost,fRegularcost,ftimeCost,fdistanceCost)
...
*/
  ...
  submitRoutesforTripsCreation = (
    ...
    for (let k = 0; k < routes.length; k++) {
      ...
      var auto_tot_travel_time = formatTime(routes[k].duration)
      var auto_total_time = Number((routes[k].duration + routes[k].service) / 60 / 60).toFixed(2)
      var auto_service_time = routes[k].service / 60 / 60
      var auto_tot_distance = Number(routes[k].distance / 1000).toFixed(2)
      ...
      for (let t = 0; t < currRoute.steps.length; t++) {
        ...
        if (
          ...
          if (currTask.type !== 'start' && currTask.type !== 'end') {
            ...
            for (let d = 0; d < this.state.docsPanel.length; d++) {
              ...
              if (currDoc.docnum === docno) {
                ...
                // )
                currDoc.arrival = timestampToTime(currTask.arrival)
                ...
                // )
                currDoc.end = timestampToTime(currTask.arrival + currTask.service)
                ttime = currDoc.arrival
                prevDocDepartTime =
          ...
          currDoc.docdate = this.getEndDate(
            this.state.documentPanel_date,
            startVehTime,
            currTask.arrival
          ).endDate
          ...
          currDoc.arrival = timestampToTime(currTask.arrival)
          // currDoc.time = convertSecToMin(currTask.duration)
          currDoc.time = convertMinToHr(prevttime).toFixed(3)
          // console.log("OSRM curr task", currTask)
          // console.log("OSRM curr task", currTask)
          //    let tempwaittime = convertHrToSec(currDoc.waitingTime)
          let tempservicetime = currTask.service //- tempwaittime;
          ...
          // console.log("OSRM start task",currTask.arrival);
          // startTime = secondsToHms(currTask.arrival)
          startTime = timestampToTime(currTask.arrival)
          prevDocDepartTime = currTask.arrival + currTask.service + currTask.waiting_time
          ttime = startTime
        } else if (currTask.type === 'end') {
          endTime = secondsToHms(currTask.arrival)
          // finalEndTime = this.adjustTime(secondsToHms(currTask.arrival))
          finalEndTime = timestampToTime(currTask.arrival)
          ttime = endTime
          // console.log("OSRM end task",currTask.arrival);
        }
        //for timeline
      ...
      if (auto_total_time > VehicleObject.overtimestar) {
        fovertimecost = (auto_total_time - VehicleObject.overtimestar) * VehicleObject.costperunito
        fRegularcost = VehicleObject.overtimestar * VehicleObject.costperunitt
        ftimeCost = fovertimecost + fRegularcost
      } else {
        fRegularcost = VehicleObject.costperunitt * auto_total_time
        ftimeCost = fRegularcost
      }
      ...
      fld_per_capacity = parseFloat((fld_doc_capacity / fld_tot_capacity) * 100).toFixed(2)
      fld_per_volume = parseFloat((fld_doc_volume / fld_tot_volume) * 100).toFixed(2)
      ...
      var trip = {
        ...
        tot_capacity: fld_tot_capacity,
        tot_volume: fld_tot_volume,
        doc_capacity: fld_doc_capacity.toFixed(2),
        doc_volume: fld_doc_volume.toFixed(2),
        per_capacity: fld_per_capacity,
        per_volume: fld_per_volume,
        uom_capacity: fld_uom_capacity,
        uom_volume: fld_uom_volume,
        ...
        totalWeight: fld_doc_capacity.toFixed(2) + ' ' + fld_uom_capacity,
        totalVolume: fld_doc_volume.toFixed(2) + ' ' + fld_uom_volume,
        travelTime: auto_tot_travel_time,
        serviceTime: auto_service_time,
        totalTime: auto_total_time,
  ...
  // without HOS breaks
  submitRoutesforTripsCreationWithoutHOS = (
    routes,
    site,
    selDocs,
    selDrivers,
          ...
          <Container fluid>
            ...
            >
              ...
              <SideNav_Test
                ...
                handleBreaksChange={this.handleBreaksChange}
                groupNextBillionsOptmiseTrips={
