...
const DataDetails = (props) => {
 ...
 const [selectedRows, setSelectedRows] = useState([]);
  ...
  let totalPages =  props.OpendocsList && Math.ceil(props.OpendocsList.length / 10);
  let startIndex = props.OpendocsList && (currentPage - 1) * 10;
  let endIndex = props.OpendocsList && startIndex + 10;
  let currentItems = props.OpendocsList &&  props.OpendocsList.slice(startIndex, endIndex);
  ...
  const RouteBadgeLink = (vrcode) => {
                             let url, content;
      url = "http://x3tms.tema-systems.com/syracuse-main/html/main.html?url=/trans/x3/erp/TMSNEW/$sessions?f=GESXX10CPLC/2//M/"+vrcode;
                                                         // content = "<div id = 'bodyContent'><a href=" + url + " target='_blank'>" +docno + "</a>"</div></div>";
                                                     return (
                                                              <a href={url} target='_blank' >{vrcode}  </a>
                                     );
  }
...
*/
  ...
  return (
    <div style={{ width : "100%",borderLeft:"2px solid blue",borderRight:"2px solid blue",borderBottom:"2px solid blue"}}  className='pb-3'>
      ...
      <Table>
        <thead>
          <tr>
            ...
            <th style={{ background: '#3498db', color: 'white' }}>Total Hours</th>
            <th style={{ background: '#3498db', color: 'white' }}>Total Distance</th>
            <th style={{ background: '#3498db', color: 'white' }}>Over Time</th>
            <th style={{ background: '#3498db', color: 'white' }}>Absence Hours</th>
