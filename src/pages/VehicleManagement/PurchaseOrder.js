...
export default function PurchaseOrderManager({
  ...
  //   const results = VehicleList.filter((order) => {
  //     const regMatch = order.registration?.toLowerCase().includes(searchTerm.toLowerCase());
  //     const codeMatch = order.code?.toLowerCase().includes(searchTerm.toLowerCase());
  //     const siteMatch = order.site?.toLowerCase().includes(searchTerm.toLowerCase());
  //     return regMatch || codeMatch || siteMatch;
  //   });
...
useEffect(() => {
  ...
  if (lowerSearchTerm === "") {
    setFilteredOrders(VehicleList);
  } else {
    const results = VehicleList.filter((order) => {
      const regValue = (order.licensePlate || "").trim().toLowerCase();
      const codeValue = (order.code || "").trim().toLowerCase();
      const siteValue = (order.site || "").trim().toLowerCase();

      const regMatch = regValue.includes(lowerSearchTerm);
      const codeMatch = codeValue.includes(lowerSearchTerm);
      const siteMatch = siteValue.includes(lowerSearchTerm);

      return regMatch || codeMatch || siteMatch;
    });

    // Check if there's a record with an exact match on registration or code
    const exactMatch = results.find(
      (order) =>
        (order.licensePlate || "").trim().toLowerCase() === lowerSearchTerm ||
        (order.code || "").trim().toLowerCase() === lowerSearchTerm
    );

    if (exactMatch) {
      // If exact match found, setFilteredOrders to only that result
  ...
  useEffect(() => {
    ...
    setErrors({
      code: selectedOrder?.code.trim() ? "This field is mandatory" : "",
      licensePlate: selectedOrder?.rlicensePlate.trim()
        ? "This field is mandatory"
        : "",
      site: selectedOrder?.site.trim() ? "This field is mandatory" : "",
      vehicleClass: selectedOrder?.vehicleClass.trim()
        ? "This field is mandatory"
        : "",
      carrier: selectedOrder?.carrier.trim() ? "This field is mandatory" : "",
      startDepotName: selectedOrder?.startDepotName.trim()
        ? "This field is mandatory"
        : "",
      endDepotName: selectedOrder?.endDepotName.trim()
        ? "This field is mandatory"
        : "",
      earliestStartTime: selectedOrder?.earliestStartTime.trim()
        ? "This field is mandatory"
        : "",
      latestStartTime: selectedOrder?.latestStartTime.trim()
        ? "This field is mandatory"
        : "",
      costPerUnitOverTime:
        selectedOrder &&
        selectedOrder.costPerUnitOverTime != undefined &&
        selectedOrder.costPerUnitOverTime != ""
          ? "This field is mandatory"
          : "",

      costPerUnitDistance:
        selectedOrder?.costPerUnitDistance !== undefined &&
        selectedOrder.costPerUnitDistance !== null &&
        selectedOrder.costPerUnitDistance !== ""
          ? "This field is mandatory"
          : "",
      totalMaxDistance:
        selectedOrder?.totalMaxDistance !== undefined &&
      ...
      maxTotalTime:
        selectedOrder?.maxTotalTime !== undefined &&
      ...
      maxTotalTravelTime:
        selectedOrder?.maxTotalTravelTime !== undefined &&
        selectedOrder.maxTotalTravelTime !== null &&
        selectedOrder.maxTotalTravelTime !== ""
          ? "This field is mandatory"
          : "",
      maxAllowedVolume:
        selectedOrder?.maxAllowedVolume !== undefined &&
        selectedOrder.maxAllowedVolume !== null &&
        selectedOrder.maxAllowedVolume !== ""
          ? "This field is mandatory"
          : "",

      style: selectedOrder?.style.trim() ? "This field is mandatory" : "",
      maxAllowedWeight:
        selectedOrder?.maxAllowedWeight !== undefined &&
      ...
      costPerUnitTime:
        selectedOrder?.costPerUnitTime !== undefined &&
        selectedOrder.costPerUnitTime !== null &&
            ...
            {currentItems.map((order) => (
              <tr
                ...
                style={{
                  ...
                  backgroundColor:
                    selectedOrder?.code === order?.code ? "lightblue" : "",
                }}
              >
                <td>{order.code}</td>
                ...
                <td>
                    ...
                    <Badge color={statusColor[order.activeFlag]} pill className="px-3 py-2">
                  { displayStatus(order.activeFlag)}
                    </Badge>
